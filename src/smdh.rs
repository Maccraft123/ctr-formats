...
impl Smdh {
    /// Safety: This function assumes that data is correct SMDH data
    /// And does no error checking, besides length and magic number
    pub unsafe fn from_u8_slice(data: &[u8]) -> Smdh {
        assert!(data.len() == 0x36c0);
        let this: Smdh = std::ptr::read(data.as_ptr() as *const _);
        assert_eq!(&this.magic(), b"SMDH");
        this
    }
